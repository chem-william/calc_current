...
def main() -> None:
    ...
    parser.add_argument("--config", default=None, help="name of the config file")
    ...
    # THIS FUCKS UP WHEN RAN IN PARALLEL!
    ...
    sigma_r = -1j / 2.0 * (GamL + GamR)  # + V_pot
    ...
    sigma_r_mo = -1j / 2.0 * (GamL_mo + GamR_mo)  # + V_pot
    ...
    Gles_mo *= bias
    np.save(
        path + data_basename + "matrices_dV_mo.npy", [Gr_approx_mo, Gles_mo, GamL_mo]
    )

    utils_zcolor.plot_complex_matrix(Gles_mo, path + "Gles_mo")

    Tt_mo = np.dot(
        np.dot(np.dot(GamL_mo, Gr_approx_mo), GamR_mo), Gr_approx_mo.T.conj()
    )
