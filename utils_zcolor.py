...
def plot_eigenchannels(
    atoms: Atoms, teig_phi_xg, folder_name: str = "./basis/eigenchannels"
) -> None:
...
def calc_trans(energy_grid, gret, gamma_left, gamma_right):
    ...
    trans = np.array(
        [
            np.matmul(
                np.matmul(
                    np.matmul(gamma_left[:, :, en], gret[:, :, en]),
                    gamma_right[:, :, en],
                ),
                gret[:, :, en].T.conj(),
            ).trace()
...
def retarded_gf(h_ao, s_ao, energy, gamma_left, gamma_right):
    ...
    """
    return np.linalg.inv(energy * s_ao - h_ao + (1j / 2.0) * (gamma_left + gamma_right))
...
def retarded_gf2(h_ao, s_ao, energy, sigma_ret):
    ...
    return np.linalg.inv(-sigma_ret + (energy + eta * 1.0j) * s_ao - h_ao)
...
def ret_gf_ongrid(energy_grid, h_ao, s_ao, gamma_left, gamma_right):
    ...
    ret_gf = np.array(
        [
            retarded_gf(
                h_ao, s_ao, energy_grid[en], gamma_left[:, :, en], gamma_right[:, :, en]
            )
            for en in range(len(energy_grid))
        ]
    )
...
def fermi_ongrid(energy_grid, e_f, bias):
    ...
    for en in energy_grid:
        f_left.append(fermi(en, e_f + bias / 2.0))
        f_right.append(fermi(en, e_f - bias / 2.0))
        ...
        for iy, y2 in enumerate(y):
            for iz, z2 in enumerate(z):
                ...
                if norm2 > cut_off and z2 > refcoord1[2] and z2 < refcoord2[2]:
                    ...
                    rel_phi = (
                        jy[ix, iy, iz]
                        * np.cos(np.arctan2(y2 - refcoord1[1], x2 - refcoord1[0]))
                        - jx[ix, iy, iz]
                        * np.sin(np.arctan2(y2 - refcoord1[1], x2 - refcoord1[0]))
                    ) / norm2
                    cyl_color = cyl_colorlist[
                        int(np.round(rel_phi, decimals=2) * 100) + 100
                    ]
...
def plot_convergence() -> None:
    ...
    current = (
        np.abs(np.real(np.load("./data/data/current_dV_mo.npy", allow_pickle=True)))
        * 6.623618183e-3
    )
    z *= 0.529177249 * 0.1
    dA = (x[1] - x[0]) * (y[1] - y[0])
