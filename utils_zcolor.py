...
def all_this(atoms, calc, path, basis, basis_full, xc,
             FDwidth, kpts, mode, h, vacuum=4):
    wfs = calc.wfs
                        ...
                        for iz, z in enumerate(z_cor):
                            ...
                            if np.sqrt(x_grid[ix, iy, iz]**2+y_grid[ix, iy, iz]**2+z_grid[ix, iy, iz]**2) > 5e-4:
                                ...
                                text_file.write("draw arrow{0} arrow color [1,0,0] diameter 0.02 {{ {1},{2},{3} }} {{ {4},{5},{6} }} \n".format(
                                    a, x*au2A, y*au2A, z*au2A, (x+x_grid[ix, iy, iz])*au2A, (y+y_grid[ix, iy, iz])*au2A, (z+z_grid[ix, iy, iz])*au2A))
                            a += 1
...
def plot_total_current(path, co, energy_grid, trans, fL, fR, atoms, h, basis, vacuum, xc, basename):
    ...
    eV2au = 27.211
    current = (1/eV2au)*(1/np.pi) * \
        np.array([trans[en].real*(fL[en]-fR[en])*dE for en in range(len(energy_grid))]).sum()
